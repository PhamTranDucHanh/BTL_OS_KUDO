 ...
 */
   ...
   uint32_t destination)
 {
   ...
   // if (size_rg == 0)
   // {
   //   printf("%s\tYêu cầu đọc không hợp lệ: reg_index %d không tồn tại%s\n",RED, source,RESET);
   // 	return -1;
   // }
 
   if (offset > size_rg)
   {
    //  printf("%s\tYêu cầu đọc không hợp lệ: reg_index %d range from %ld to %ld but you read at %ld%s\n",
    //       RED,
    //       source,
 ...
 #ifdef IODUMP
   printf("\tĐọc region=%d offset=%d value=%d\n", source, offset, data);
 #ifdef PAGETBL_DUMP
...
//    // if (size_rg == 0)
//    // {
//    //   printf("%s\tYêu cầu ghi không hợp lệ: reg_index %d không tồn tại%s\n",RED, destination,RESET);
//    // 	return -1;
//    // }
...
//     //  printf("%s\tYêu cầu ghi không hợp lệ: reg_index %d range from %ld to %ld but you write at %ld%s\n",
//     //       RED,
//     //       destination,
...
//    }
//  #ifdef IODUMP
//    printf("\tGhi region=%d offset=%d value=%d\n", destination, offset, data);
//  #ifdef PAGETBL_DUMP
//    print_pgtbl(proc, 0, -1); // print max TBL
//  #endif
//    MEMPHY_dump(proc->mram);
...
//  }
 ...
 int get_free_vmrg_area(struct pcb_t *caller, int vmaid, int size, struct vm_rg_struct *newrg)
 {
   struct vm_area_struct *cur_vma = get_vma_by_num(caller->mm, vmaid);
   if (cur_vma == NULL) {
    printf("[get_free_vmrg_area] cur_vma is NULL (vmaid = %d)\n", vmaid);
